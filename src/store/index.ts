...
export type RootState = ReturnType<typeof store.getState> // получаем тип состояния // этот тип будет знать о тех редюсерах с которыми мы работаем, и о тех данных, с которыми мы работаем, т.е. состояния отдельного редюсера
